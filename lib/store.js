...
export const useStore = create((set, get) => ({
  ...
  removeCityAuth: async (cityName) => {
    try {
      const res = await fetch(`/api/user/cities?name=${encodeURIComponent(cityName)}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  updateProfile: async (userData, signal) => {
    // 1. Guardar estado anterior por si falla o cancela
    const previousUser = get().user;
    
    // 2. Actualización optimista en el estado global
    ...
    // 3. Ejecutar la petición, ahora esperando la respuesta real para controlar el modal
    try {
      const res = await fetch('/api/user/profile', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(userData),
        signal: signal // <-- para poder abortar si cierran la ventana
      });
      ...
      if (err.name === 'AbortError') {
        // Revertir si el usuario canceló
        set({ user: previousUser });
        get().showToast('Actualización cancelada', 'success');
        return false;
      }
      console.error(err);
      set({ user: previousUser });
