...
export const useStore = create((set, get) => ({
  ...
  updateProfile: async (userData) => {
    ...
    // 3. Petición en segundo plano para no bloquear la UI (el modal se cerrará al instante)
    (async () => {
      try {
        ...
        if (res.ok) {
          // Consolidar datos con la respuesta final del servidor
          set({ user: data.user });
    ...
    // 4. Retornar true inmediatamente para que el modal se cierre al instante sin esperar a la BD
    return true;
  },
